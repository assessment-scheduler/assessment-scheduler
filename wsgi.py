...
from App.controllers.course import (assign_lecturer, create_course, get_all_course_codes, get_all_courses, get_course)
...
from App.controllers.semester import create_semester, get_all_semesters, get_semester, get_semester_duration, set_active
from App.views.kris import compile_course_data, compute_schedule, schedule_all_assessments
from App.controllers.courseoverlap import ( get_all_cells, get_cell, get_course_row, get_course_matrix, get_phi_matrix)
...
def list_all_assessments():
    ...
    for assessment in assessments:
        table.add_row([assessment.course_code, assessment.name, f"{assessment.percentage}%", assessment.scheduled]) 
    print(table)

@assessment_cli.command("by_course", help = "returns all assessments for a course in json format")
@click.argument("course_code")
...
def list_overlaps():
    ...
    table = PrettyTable()
    table.field_names = ["Course Code 1", "Course Code 2", "Overlap"]  # Adjust fields as necessary
    for overlap in overlaps:
        table.add_row([overlap.code1, overlap.code2, overlap.student_count])  # Fixed attribute names
    print(table)
...
def create_semester_command(start_date: str, end_date: str, sem_num: int, max_assessments: int) -> None:
    iso_start_date: date = date.fromisoformat(start_date)
    iso_end_date: date = date.fromisoformat(end_date)
    success: bool = create_semester(iso_start_date, iso_end_date, sem_num, max_assessments)
    if success:
        print(f"Semester {sem_num} created successfully from {start_date} to {end_date}")
    else:
        print(f"Failed to create semester {sem_num}. It may overlap with an existing semester.")

@semester_cli.command("list", help="List all semesters")
def list_semesters() -> None:
    semesters = get_all_semesters()
    table = PrettyTable()
    table.field_names = ["ID", "Start Date", "End Date", "Semester Number", "Max Assessments", "Constraint Value", "Active"]
    for semester in semesters:
        table.add_row([semester.id, semester.start_date, semester.end_date, semester.sem_num, semester.max_assessments, semester.constraint_value, semester.active])
    print(table)
